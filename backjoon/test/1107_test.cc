...
int solve()
{
    ...
    if (n == 199) {
        std::cout << "diff_100: " << diff_100 << "\n";
        std::cout << "same_len: " << same_len << "\n";
        std::cout << "short_max: " << short_max << "\n";
...
{
    int i, temp, act, ans;
    std::ifstream ifs("./testcase/1107_tc.txt", std::ios::in);
    do {
        ifs >> n >> k;
        if (ifs.eof())
            break;
